 ...
 * question and must never both appear, Delete is offered on everything that is
 * not already deleted, and Edit, Finish and Stop only make sense while a course
...
export function courseActions(g: MedicineGroup, ref: DateKey = today()): CourseAction[] {
  ...
  if (status === 'active') return ['edit', 'finish', 'stop', 'delete']
  if (status === 'upcoming') return ['edit', 'stop', 'delete']
