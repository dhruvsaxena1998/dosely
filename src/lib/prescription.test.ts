...
describe('the prescription as text', () => {
  it('prints a course as a name and an indented block under it', () => {
    expect(prescriptionText([{ title: 'Running', groups: [metformin] }], '2026-09-16')).toBe(
      [
        'Dosely · 16 Sep 2026',
        '',
        'RUNNING',
  ...
  it('leaves a course with no note at three lines', () => {
    const text = prescriptionText([{ title: 'Not started', groups: [vitaminD] }], '2026-09-16')
  ...
  it('says nothing at all when every section is empty', () => {
    expect(prescriptionText([{ title: 'Running', groups: [] }], '2026-09-16')).toBe('')
    expect(prescriptionText([], '2026-09-16')).toBe('')
  })
